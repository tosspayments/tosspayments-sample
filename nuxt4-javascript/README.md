> ⚠️ **주의사항 (계좌자동결제/퀵계좌이체 빌링 테스트)**  
> 이 샘플에서는 계좌자동결제(퀵계좌이체 빌링) 테스트를 위해 발급된 빌링키를 서버 메모리의 `billingKeyMap` 에 저장합니다.  
> 따라서 서버 재시작 시 빌링키가 유실되고, 다중 인스턴스(스케일 아웃) 환경에서는 인스턴스 간에 빌링키가 공유되지 않습니다.  
> **운영 환경에서는 반드시 DB, Redis 등 영구/공유 가능한 저장소에 빌링키를 저장**하도록 구현해야 합니다.  
